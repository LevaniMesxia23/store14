...
function Header() {
  ...
  return (
    <header
      className={`${
        i18n.language === "en" ? "font-righteous" : "font-mikheil"
      ...
      <div className="flex items-center justify-between px-8 py-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative -mb-[7px]" onClick={toggleDropdown}>
            ...
            />
            <div
              className={`z-20 absolute top-full mt-4 bg-transparent border -ml-4 shadow-lg rounded-lg p-2 transition-all duration-300 ease-in-out overflow-hidden ${
                isDropdownOpen
        ...
        >
          <ul className="flex flex-col  space-y-4 md:mt-0 md:flex-row md:space-y-0 md:space-x-4">
            {navLinks.map((link) => (
              <li
                key={link.name}
                className="hover:bg-[#333333] pl-10 pt-6 text-[1.8rem] font-bold"
              >
                <Link
                  to={link.path}
